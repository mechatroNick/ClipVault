...
def add_file_to_pbxproj(proj_path, file_path, group_name, target_name):
    ...
    file_ref_id = f"P4_PREVIEW_{file_name.upper().replace('.', '_')}_REF"
    build_file_id = f"P4_PREVIEW_{file_name.upper().replace('.', '_')}_BUILD"
    ...
    # Find or create Previews group
    if f'/* {group_name} */' not in content:
        print(f"Group {group_name} not found. Adding to Views.")
        group_name = "Views"
...
previews = [
    'ClipVault/Views/Previews/TextPreview.swift',
    'ClipVault/Views/Previews/ImagePreview.swift',
    'ClipVault/Views/Previews/FilePreview.swift',
    'ClipVault/Views/Previews/URLPreview.swift',
    'ClipVault/Views/Previews/ContentPreviewRouter.swift'
]
for p in previews:
    add_file_to_pbxproj(proj, p, 'Views', 'ClipVault')

# Add test file
add_file_to_pbxproj(proj, 'Tests/ClipVaultTests/ContentPreviewTests.swift', 'ClipVaultTests', 'ClipVaultTests')
