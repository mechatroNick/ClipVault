...
def add_file_to_pbxproj(proj_path, file_path, group_name, target_name):
    ...
    file_ref_id = f"P4U_{file_name.upper().replace('.', '_')}_REF"
    build_file_id = f"P4U_{file_name.upper().replace('.', '_')}_BUILD"
...
# Add files I need
# add_file_to_pbxproj(proj, 'ClipVault/Views/HistoryPanelView.swift', 'Views', 'ClipVault') # Already there
