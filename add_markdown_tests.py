...
def add_file_to_pbxproj(proj_path, file_path, group_name, target_name):
    ...
    file_ref_id = f"PMT_{file_name.upper().replace('.', '_')}_REF"
    build_file_id = f"PMT_{file_name.upper().replace('.', '_')}_BUILD"
...
add_file_to_pbxproj(proj, 'Tests/ClipVaultTests/MarkdownRendererTests.swift', 'ClipVaultTests', 'ClipVaultTests')
